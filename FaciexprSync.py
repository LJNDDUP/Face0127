...
parser.add_argument('--test_dir',     type=str, help='directory to store testing data',  default='./test')
args = parser.parse_args()

if __name__ == '__main__':
    p_idx_files = os.listdir(args.test_dir)
    ...
    loadmodel_directory = './checkpoint/0122_r0.5_smooth0.5/epoch_510batch_22430_lossr_0.044969.pkl'
    ...
    for p_idx_file in p_idx_files:
        if p_idx_file.find('_p_idx.txt') > 0:
            p_idx = open(os.path.join(args.test_dir, p_idx_file), 'r', encoding='utf-8-sig').readlines()[0]
            ...
            sync_skeleton_f = open(os.path.join(args.test_dir, p_idx_file).replace('p_idx', 'sync_blendshape'), 'w')
            writein = np.zeros((51,))
